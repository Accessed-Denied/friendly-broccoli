...
class CreatePost: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
    ...
    private func configUI(){
        ...
        postTextView.layer.borderColor = colorFromHex(hex: "#CACED1").cgColor
    ...
    @IBAction func clickPostBtn(_ sender: UIButton) {
        if categoryBtn.currentTitle == "Select Category"{
            ...
            displayToast("Please select media type")
        }
        else{
            if isImagePost{
                User.CreatePost(userId: AppModel.shared.loggedInUser.id, username: AppModel.shared.loggedInUser.name, postType: postType, categoryName: categoryName, postText: postTextView.text, mediaType: POSTMEDIA.IMAGE, postMediaData: compressImage(image: PostImage ?? UIImage()), videoUrl: nil) { (loginHandler) in
                    if loginHandler == nil{
                ...
                User.CreatePost(userId: AppModel.shared.loggedInUser.id, username: AppModel.shared.loggedInUser.name, postType: postType, categoryName: categoryName, postText: postTextView.text, mediaType: POSTMEDIA.VIDEO, postMediaData: compressImage(image: PostImage ?? UIImage()), videoUrl: self.videoUrl) { (loginHandler) in
                    if loginHandler == nil{
                        displayToast("\(self.categoryName) Post created successfully")
                        self.dismiss(animated: true, completion: nil)
                    }
